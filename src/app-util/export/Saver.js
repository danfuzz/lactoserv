...
export class Saver {
   ...
   * just before the runner stops. If the subclass doesn't need to do anything,
   * it is safe to _not_ override this method.
   */
  async _impl_doWork() {
    // Nothing to do here.
  }
   ...
   * If the concrete subclass needs to perform occasional work synchronously
   * with respect to the threadlet which runs this instance, this method should
   * async-return (or return a promise which gets resolved) when that work ought
   * next to be performed. If the subclass never has work to do, it is safe to
   * _not_ override this method.
   */
  ...
  async #findFiles(options = {}) {
    ...
    const todayStr = Saver.#makeInfix(new Date());
    const contents = await fs.readdir(directory);
    const result   = [];

    for (const name of contents) {
      const parsed = Saver.#parseInfix(name, filePrefix, fileSuffix);
      if (parsed === null) {
        continue;
      }
  ...
  async #targetPath(stats) {
    const dateStr = Saver.#makeInfix(stats.birthtime);
    const resolve = (count) => {
      const infix = Saver.#makeInfix(dateStr, (count > 0) ? count : null);
    ...
    if (this.#lastInfix === dateStr) {
      ...
      if (!await Saver.#fileExists(firstTry)) {
