...
// Note: Node's `fetch()` often seems to put unexpected headers into the
// requests, making it arguably a bad choice for these tests which all ideally
// exert careful control over what's actually sent. There is a lot of code in
// this file which tries to hide this stuff from the callers, but the situation
// is less than ideal. TODO: Replace `fetch()` with something different (though,
// for ease, perhaps with a compatible API).

// Used to when figuring out whether to expect `fetch()` to use HTTP2.
const nodeMajorVersion =
  parseInt(process.version.match('^v(?<major>[0-9]+)')?.groups.major);
...
function mergeConfigs(fromCaller, underlay) {
  ...
  for (const [k, v] of Object.entries(result)) {
    // We don't mess with `body`, because `body: null` is a proactive
    // expectation of no-body from the caller.
    if ((v === null) && (k !== 'body')) {
      delete result[k];
    }
  }
...
export async function checkResponse(responsePromise, expected) {
  ...
  // We can't control how `fetch()` deals with connection management (see
  // comment at the top of the file), so we just drop the salient headers from
  // the response.
  ...
  expected = mergeConfigs(expected, {
    headers: {
      'date':   /^[ ,:0-9A-Za-z]+ GMT$/,
      'server': /^lactoserv-[.0-9]+ [0-9a-f]+(?: [-_.0-9a-zA-Z]+)*$/
    }
  });
  ...
  // HTTP2 has no status text, so don't check `statusText` for it.
  const expectHttp2 = expected.expectHttp2AsOfNode26 && (nodeMajorVersion >= 26);
  if (!expectHttp2) {
  ...
  const gotHeaders = new Map(responseHeaders.entries());
  ...
  if (bodyCheck === undefined) {
    ...
    if (gotBody === null) {
      console.log('No body, as expected.');
    } else {
      console.log('Got unexpected body.\n');
      printBody();
    }
  } else {
    if (gotBody === null) {
...
export async function usualFetch(options) {
  const { url, ...finalOptions } = mergeConfigs(options, {
    ...
    headers: {
      ...
      // `cache-control: no-cache`, which is defined to make conditional
      // requests always fail. Since many of these tests test exactly that sort
      // of functionality, we err on the side of providing a header that allows
